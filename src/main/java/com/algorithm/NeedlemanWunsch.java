...
public class NeedlemanWunsch {
    ...
    public static OptimalAlignment obtainOptimalAlignmentByDownmostOrder(float similarityMatrix[][],
        ...
        while (i != 0 || j != 0) {
            if (j!=0&&similarityMatrix[i][j] == (similarityMatrix[i][j - 1] + indelScore)) {
                alignment1Builder.insert(0, "-");
                alignment2Builder.insert(0, sequence2.charAt(j));
                j--;
            } else if (j!=0&&i!=0&&similarityMatrix[i][j] == (similarityMatrix[i - 1][j - 1] + (sequence1.charAt(i) == sequence2.charAt(j) ? matchScore : mismatchScore))) {
                alignment1Builder.insert(0, sequence1.charAt(i));
                alignment2Builder.insert(0, sequence2.charAt(j));
                i--;
                j--;
            } else if (i!=0&&similarityMatrix[i][j] == (similarityMatrix[i - 1][j] + indelScore)){
                alignment1Builder.insert(0, sequence1.charAt(i));
